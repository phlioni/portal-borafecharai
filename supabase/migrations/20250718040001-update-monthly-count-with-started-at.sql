...
-- Atualizar função get_monthly_proposal_count para usar subscription_started_at
...
BEGIN
  ...
  -- Para outros planos ou quando não tem subscription_started_at, retornar contagem total
  SELECT COUNT(*)::INTEGER INTO monthly_count
  FROM public.proposals
  WHERE user_id = _user_id
    AND DATE_TRUNC('month', created_at)::DATE = DATE_TRUNC('month', _month)::DATE;
