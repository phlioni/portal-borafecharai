...
-- Atualizar função can_create_proposal para corrigir lógica de permissões
...
-- Atualizar trigger para garantir que admin principal sempre tenha role admin
...
BEGIN
  ...
  ELSE
    -- Para outros usuários, criar role user
    INSERT INTO public.user_roles (user_id, role)
    VALUES (NEW.id, 'user')
    ON CONFLICT (user_id, role) DO NOTHING;
    RAISE LOG 'Role user atribuída para usuário: %', NEW.id;
  END IF;
