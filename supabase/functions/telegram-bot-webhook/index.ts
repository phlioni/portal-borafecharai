...
async function sendTelegramMessage(chatId, text, replyMarkup = {}) {
  ...
  const payload = { chat_id: chatId, text, parse_mode: 'HTML', reply_markup: replyMarkup, disable_web_page_preview: true };
  try {
...
async function showProposalSummary(session, chatId) {
  await supabase.from('telegram_sessions').update({ step: 'awaiting_proposal_confirmation' }).eq('id', session.id);
  ...
  await supabase.from('telegram_sessions').update({ session_data: session.session_data }).eq('id', session.id);

  await sendTelegramMessage(chatId, summary, {
    inline_keyboard: [
      [{ text: '✅ Salvar como Rascunho', callback_data: 'proposal_confirm:save' }],
      [{ text: '🚀 Salvar e Enviar por E-mail', callback_data: 'proposal_confirm:send' }]
    ]
  });
}
...
async function transcribeAudio(file_id: string): Promise<string> {
...
async function extractProposalDetails(text: string): Promise<any> {
...
async function showAiSummaryAndAskForEdit(session, chatId) {
  ...
  await supabase.from('telegram_sessions').update({ step: 'awaiting_edit_confirmation', session_data: session.session_data }).eq('id', session.id);
  await sendTelegramMessage(chatId, summary, {
    inline_keyboard: [
      [{ text: '✅ Sim, está correto!', callback_data: 'edit_proposal:confirm' }],
      [{ text: '✏️ Não, quero editar', callback_data: 'edit_proposal:start_edit' }]
    ]
  });
}
    ...
    if (update.callback_query) {
      ...
      try {
        ...
        if (action === 'main_choice') {
          if (value === 'create_proposal') {
            await supabase.from('telegram_sessions').update({
              ...
              session_data: { proposal: { budget_items: [] } },
            }).eq('id', session.id);
            ...
            const { data: proposals, error } = await supabase
              .from('proposals').select(`id, title, value, status, client:clients (name)`).eq('user_id', session.user_id)
            ...
            if (error || !proposals || proposals.length === 0) {
              ...
              for (const p of proposals) {
                const statusMap = {
                  rascunho: 'Rascunho', enviada: 'Enviada', visualizada: 'Visualizada', aceita: 'Aceita', rejeitada: 'Rejeitada'
                };
                const statusText = statusMap[p.status] || p.status.charAt(0).toUpperCase() + p.status.slice(1);
                let statusEmoji = '';
                if (p.status === 'rascunho') { statusEmoji = '📝 '; }
                else if (p.status === 'enviada') { statusEmoji = '📧 '; }

                proposalList += `${statusEmoji}<b>${p.title}</b>\n`;
                proposalList += `<i>Status:</i> ${statusText}\n`;
                if (p.client) proposalList += `<i>Cliente:</i> ${p.client.name}\n`;
                proposalList += `<i>Valor:</i> R$ ${p.value?.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) || 'N/D'}\n`;
        ...
        if (action === 'use_template') {
          if (value === 'none') {
            await supabase.from('telegram_sessions').update({ step: 'awaiting_budget_item' }).eq('id', session.id);
            ...
            if (!template || !template.items || template.items.length === 0) {
              ...
              templateItems.forEach(item => { listMessage += `• ${item.description}\n`; });
        ...
        if (action === 'save_template') {
          if (value === 'yes') {
            const { count } = await supabase.from('budget_templates').select('*', { count: 'exact', head: true }).eq('user_id', session.user_id);
            if (count !== null && count >= 15) {
              ...
              await supabase.from('telegram_sessions').update({ step: 'awaiting_template_name' }).eq('id', session.id);
        ...
        if (action === 'confirm_client') {
          if (value === 'yes') {
            ...
            await sendTelegramMessage(chatId, `Cliente definido! 👍\n\nComo você prefere detalhar a proposta?`, {
              inline_keyboard: [
                [{ text: '🎙️ Enviar um Áudio', callback_data: 'proposal_method:audio' }],
                [{ text: '✍️ Digitar passo a passo', callback_data: 'proposal_method:text' }]
              ]
            });
        ...
        if (action === 'proposal_method') {
          if (value === 'audio') {
            await supabase.from('telegram_sessions').update({ step: 'awaiting_proposal_audio' }).eq('id', session.id);
            ...
            await supabase.from('telegram_sessions').update({ step: 'awaiting_service_summary' }).eq('id', session.id);
        ...
        if (action === 'edit_proposal') {
          if (value === 'confirm') {
            ...
            // --- AJUSTE: Pergunta COMO o usuário quer editar ---
            await supabase.from('telegram_sessions').update({ step: 'awaiting_edit_method_choice' }).eq('id', session.id);
            await sendTelegramMessage(chatId, 'Como você prefere corrigir as informações?', {
              inline_keyboard: [
                [{ text: '🎙️ Gravar novo áudio', callback_data: 'edit_method:audio' }],
                [{ text: '✏️ Editar campos por texto', callback_data: 'edit_method:text' }]
              ]
            });
          }
        }
        ...
        if (action === 'edit_method') {
          if (value === 'audio') {
            // Limpa os dados extraídos pela IA para recomeçar
            ...
            session.session_data = { ...rest, proposal: { ...proposalRest, budget_items: [] } };
            ...
            await supabase.from('telegram_sessions').update({ step: 'awaiting_edit_choice' }).eq('id', session.id);
            await sendTelegramMessage(chatId, 'O que você gostaria de editar?', {
              inline_keyboard: [
                [{ text: 'Escopo (Resumo)', callback_data: 'edit_choice:summary' }],
                [{ text: 'Prazo de Entrega', callback_data: 'edit_choice:delivery_time' }],
                [{ text: 'Validade (dias)', callback_data: 'edit_choice:validity_days' }],
                [{ text: 'Pagamento', callback_data: 'edit_choice:payment_terms' }],
                [{ text: 'Itens do Orçamento', callback_data: 'edit_choice:budget_items' }]
              ]
            });
          }
        }
        ...
        if (action === 'edit_choice') {
          const fieldMap = {
            summary: { step: 'awaiting_new_scope_value', prompt: 'Digite o novo <b>Escopo (resumo)</b>:' },
            delivery_time: { step: 'awaiting_new_delivery_time_value', prompt: 'Digite o novo <b>Prazo de Entrega</b>:' },
            validity_days: { step: 'awaiting_new_validity_days_value', prompt: 'Digite a nova <b>Validade (apenas dias)</b>:' },
            payment_terms: { step: 'awaiting_new_payment_terms_value', prompt: 'Digite as novas <b>Formas de Pagamento</b>:' },
            budget_items: { step: 'awaiting_budget_item', prompt: 'Ok, vamos refazer os itens. Envie um ou mais itens separados por ponto e vírgula (<b>;</b>).\n\n<u>Formato por item:</u>\n<code>Tipo, Descrição, Qtd, Valor (ex: 25.50)</code>\n\n<b>IMPORTANTE:</b> O "Tipo" deve ser <b>Material</b> ou <b>Serviço</b>.' }
          };
          ...
          if (editSelection) {
            ...
            await supabase.from('telegram_sessions').update({ step: editSelection.step, session_data: session.session_data }).eq('id', session.id);
        ...
        if (action === 'proposal_confirm') {
          ...
          if (proposalData.new_client && !clientId) {
            const { data: newClient, error: clientError } = await supabase.from('clients').insert({ name: proposalData.new_client.name, email: proposalData.new_client.email, phone: proposalData.new_client.phone, user_id: session.user_id }).select().single();
          ...
          await supabase.from('telegram_sessions').update({ step: 'authenticated', session_data: { email: session.session_data.email } }).eq('id', session.id);
        }
    ...
    if (update.message) {
      ...
      if (text === '/start') {
        if (session && !sessionExpired && session.user_id) {
          ...
          await sendTelegramMessage(chatId, `Olá de volta, ${currentUserName}! O que deseja fazer?`, { inline_keyboard: [[{ text: '➕ Criar Nova Proposta', callback_data: 'main_choice:create_proposal' }], [{ text: '📄 Ver Últimas Propostas', callback_data: 'main_choice:view_proposals' }]] });
        } else {
          await supabase.from('telegram_sessions').upsert({ chat_id: chatId, telegram_user_id: telegramUserId, step: 'awaiting_email', session_data: {}, expires_at: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString() }, { onConflict: 'chat_id, telegram_user_id' });
      ...
      switch (session.step) {
        ...
        case 'awaiting_email':
          ...
          await supabase.from('telegram_sessions').update({ user_id: subscriber.user_id, step: 'authenticated', session_data: { email } }).eq('id', session.id);
          await sendTelegramMessage(chatId, `✅ <b>Bem-vindo(a) de volta, ${userName}!</b>`, { inline_keyboard: [[{ text: '➕ Criar Nova Proposta', callback_data: 'main_choice:create_proposal' }], [{ text: '📄 Ver Últimas Propostas', callback_data: 'main_choice:view_proposals' }]] });
          break;

        case 'awaiting_proposal_title':
          session.session_data.proposal.title = text;
          await supabase.from('telegram_sessions').update({ step: 'awaiting_client_info', session_data: session.session_data }).eq('id', session.id);
        ...
        case 'awaiting_client_info':
          ...
          const { data: existingClient, error: searchError } = await supabase
            .from('clients').select('*').eq('user_id', session.user_id).or(`email.eq.${clientEmail}${phoneToSearch ? `,phone.eq.${phoneToSearch}` : ''}`).maybeSingle();
          ...
          if (existingClient) {
            ...
            await sendTelegramMessage(chatId, foundMessage, {
              inline_keyboard: [
                [{ text: '✅ Sim, é este', callback_data: 'confirm_client:yes' }],
                [{ text: '❌ Não, digitar novamente', callback_data: 'confirm_client:no' }]
              ]
            });

          } else {
            session.session_data.proposal.new_client = { name: clientName, email: clientEmail, phone: phoneToSearch };
            session.session_data.proposal.client_name = clientName;

            await supabase.from('telegram_sessions').update({ step: 'awaiting_proposal_method', session_data: session.session_data }).eq('id', session.id);
        ...
        case 'awaiting_proposal_audio':
          ...
          try {
            ...
            session.session_data.proposal = { ...session.session_data.proposal, ...extractedData, budget_items: extractedData.budget_items || [] };
        ...
        case 'awaiting_service_summary':
          session.session_data.proposal.summary = text;
          await supabase.from('telegram_sessions').update({ step: 'awaiting_delivery_time', session_data: session.session_data }).eq('id', session.id);
          await sendTelegramMessage(chatId, 'Entendido. Qual o <b>prazo de entrega</b>? (ex: 15 dias úteis)');
          break;

        case 'awaiting_delivery_time':
          session.session_data.proposal.delivery_time = text;
          await supabase.from('telegram_sessions').update({ step: 'awaiting_proposal_validity', session_data: session.session_data }).eq('id', session.id);
          await sendTelegramMessage(chatId, 'Ok. E qual a <b>validade da proposta</b> em dias? (ex: 10)');
          break;

        case 'awaiting_proposal_validity':
          const days = parseInt(text, 10);
          if (isNaN(days)) { await sendTelegramMessage(chatId, '❌ Por favor, envie apenas o número de dias.'); break; }
          session.session_data.proposal.validity_days = days;
          await supabase.from('telegram_sessions').update({ step: 'awaiting_payment_method', session_data: session.session_data }).eq('id', session.id);
        ...
        case 'awaiting_payment_method':
          ...
          const { data: templates, count } = await supabase.from('budget_templates').select('id, name', { count: 'exact' }).eq('user_id', session.user_id).limit(15);
          ...
          if (templates && templates.length > 0) {
            ...
            const buttons = templates.map((t) => {
              ...
              return [{ text: buttonText, callback_data: `use_template:${t.id}` }];
            });
            const keyboard = [...buttons, [{ text: '➕ Adicionar itens manualmente', callback_data: 'use_template:none' }]];
        ...
        case 'awaiting_budget_item':
          ...
          for (const itemString of itemStrings) {
            ...
            if (parts.length !== 4) { errors.push(`Formato inválido: "${itemString}"`); continue; }
            ...
            if (normalizedType.startsWith('material')) { dbType = 'material'; }
            else if (normalizedType.startsWith('servico')) { dbType = 'labor'; }
            else { errors.push(`Tipo inválido no item "${description}" (use Material ou Serviço)`); continue; }
            const quantity = parseInt(quantityStr, 10);
            const unit_price = parseFloat(unitPriceStr.replace(',', '.'));
            if (isNaN(quantity) || isNaN(unit_price)) { errors.push(`Valor/Qtd inválido no item "${description}"`); continue; }
            addedItems.push({ type: dbType, description, quantity, unit_price });
          }
          if (errors.length > 0) {
          ...
          if (session.session_data.editing_ia) {
            ...
            await sendTelegramMessage(chatId, 'Gostaria de salvar este grupo de itens como um novo Modelo de Orçamento para usar no futuro?', {
              inline_keyboard: [
                [{ text: '👍 Sim, salvar', callback_data: 'save_template:yes' }],
                [{ text: '👎 Não, ir para o resumo', callback_data: 'save_template:no' }]
              ]
            });
          }
          break;
        ...
        case 'awaiting_template_item_details':
          ...
          if (details.length !== 2) { await sendTelegramMessage(chatId, '❌ Formato inválido. Envie <code>Quantidade, Valor (ex: 25.50)</code>'); break; }
          const qty = parseInt(details[0], 10);
          const price = parseFloat(details[1].replace(',', '.'));
          if (isNaN(qty) || isNaN(price)) { await sendTelegramMessage(chatId, '❌ Quantidade ou valor inválido.'); break; }
          proposalData.budget_items.push({ type: currentItem.type, description: currentItem.description, quantity: qty, unit_price: price });
          ...
          if (nextIndex < proposalData.template_items.length) {
            ...
            await supabase.from('telegram_sessions').update({ step: 'awaiting_save_template_choice' }).eq('id', session.id);
        ...
        case 'awaiting_template_name':
          ...
          const { data: newTemplate, error: templateError } = await supabase.from('budget_templates').insert({ name: templateName, user_id: session.user_id }).select().single();
